...
- MVC 과제 NodeJS + TypeScript + Express + MySQL 5.7

----------
**필요사항**
- MySQL 5.7 환경이 필요합니다.
- 개발 환경 및 실행 환경은 Mac OS Sonoma 입니다.
- 첨부된 데이터베이스 백업 파일을 통하여 레코드 값과 view 테이블 환경을 복원시킬 수 있습니다.
...
데이터베이스 복원 방법
    ...
    mysql -u <사용자 명칭> -p <데이터베이스 명칭> < 파일명.sql
...
별도의 맵핑 테이블이 존재하여 view 테이블을 사용하여 조회할 수 있도록 설계하였습니다.
강사명 / 강의명 / 식별자 OR 로 식별하며 3중 선택하여 검색하는 것으로 해석하였습니다.
정렬 방법과 카테고리는 
페이징은 sql betewn and 혹은 limit opset 중 후자로 구현하였습니다.

### 강의 상세 조회

강의 식별자를 query 파라미터로 받아 인식하도록 설계하였으나 
URL Parameter을 통해 강의 식별자를 획득하고, HTTP 메서드로 기능을 구분하는 것이 좋을 것으로 보입니다.

### 강의 등록 / 강의 대량 등록
아쉬운점 - API를 분리하여 설계하여보았지만, 통합하여 사용하는 것 또한 좋았을 것으로 보입니다.

기본적인 설계는 body 값에서 json 배열을 받아 class 배열로 만들어 저장하여 map으로 순회하는 방식으로 설계하였습니다.
map으로 반환받은 프로미스 객체를 Promise.all로 병렬 처리를 할 수 있도록 작성하여 처리 속도를 높였습니다.

### 강의 수정
별도의 강의 수정 폼에서 값을 수정한다는 가정하에 강의명 강의 소개 가격 데이터를 필수값으로 상정하고 작성하였습니다.
기존 검증로직은 모든 강의에서 강사 식별자를 받아 강사 인증 공통 로직을 거친 후 동작하는 것을 방향으로 구현하였으나,
도메인 설계 단계에서 직관적인 url 설계가 아닌 난잡한 방향으로 작성하여 공통 로직은 비활성화 하였습니다.
router.use(lectureController.validationLecturer)

query 파라미터에 lecturer_Id = 값 을 추가하여 사용할 수 있습니다.

### 강의 오픈 / 삭제 / 수강생 탈퇴
단순 update insert문으로 트랜젝션을 열어 예기치 못한 문제가 발생하는 것을 방지하였습니다.

### 수강생 가입
중복을 허용하지 않도록 닉네임과 이메일에 각각 유니크 제약사항을 추가하였습니다.

### 수강생 수강 신청
강의 등록 부분을 재사용하여, body 값을 배열로 받아 처리할 수 있도록 설계하였습니다.
수강생 식별자는 별도로 받아 
비공개 강의 수강할 수 없도록 함수를 작성하였습니다.
가입된 강의를 재수강하는 경우 DB 유니크 제약을 위배한 에러가 발생 catch 단계에서 해당 메시지
확인하여 return 메시지를 변경합니다.
